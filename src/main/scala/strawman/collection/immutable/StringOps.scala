...
class StringOps(val repr: String)
  extends AnyVal
  ...
  protected[this] def coll = StringView(repr)
  protected[this] def c: String = repr
  protected[this] def seq: Seq[Char] = iterableFactory.fromIterable(coll)

  protected[this] def fromSpecificIterable(coll: collection.Iterable[Char]): String = {
  ...
  def apply(i: Int) = repr.charAt(i)

  override def knownSize = repr.length
  ...
  def ++(xs: IterableOnce[Char]): String = {
    val sb = new StringBuilder() ++= repr
    for (ch <- xs.iterator()) sb += ch
    sb.result()
  }

  /** Another overloaded version of `++`. */
  def ++(xs: String): String = repr + xs
