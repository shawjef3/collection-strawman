...
private[collection] trait Wrappers {
  ...
  case class JIterableWrapper[A](underlying: jl.Iterable[A]) extends AbstractIterable[A] with Iterable[A] {
    ...
    protected[this] def fromSpecificIterable(coll: Iterable[A]) = mutable.ArrayBuffer.fromIterable(coll)
  ...
  case class JListWrapper[A](underlying: ju.List[A]) extends mutable.AbstractBuffer[A] with SeqOps[A, mutable.Buffer, mutable.Buffer[A]] {
    ...
    def filterInPlace(p: A => Boolean): this.type = { underlying.removeIf(x => p(x)); this }
  ...
  case class JSetWrapper[A](underlying: ju.Set[A]) extends mutable.AbstractSet[A] with mutable.Set[A] with mutable.SetOps[A, mutable.Set, mutable.Set[A]] {
    ...
    protected[this] def fromSpecificIterable(coll: Iterable[A]) = mutable.HashSet.fromIterable(coll)
  ...
  trait JMapWrapperLike[A, B, +Repr <: mutable.Map[A, B] with mutable.Map[A, B]] extends mutable.Map[A, B] with mutable.MapOps[A, B, mutable.Map, mutable.Map[A, B]] {
    ...
    protected[this] def fromSpecificIterable(coll: Iterable[(A, B)]) = mutable.HashMap.fromIterable(coll)
    protected[this] def newSpecificBuilder() = mutable.HashMap.newBuilder()
    def mapFactory = mutable.HashMap
    protected[this] def mapFromIterable[K2, V2](it: Iterable[(K2, V2)]) = mutable.HashMap.fromIterable(it)
  }
  ...
  */
  ...
  case class JPropertiesWrapper(underlying: ju.Properties) extends mutable.AbstractMap[String, String]
    ...
    protected[this] def fromSpecificIterable(coll: Iterable[(String, String)]) = mutable.HashMap.fromIterable(coll)
    protected[this] def newSpecificBuilder() = mutable.HashMap.newBuilder()
    def mapFactory = mutable.HashMap
    protected[this] def mapFromIterable[K2, V2](it: Iterable[(K2, V2)]) = mutable.HashMap.fromIterable(it)
  }
